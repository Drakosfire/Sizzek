...
export class UserAwareTodoodlesManager {
    ...
    async saveTodoData(data, userId) {
        const effectiveUserId = userId || this.getUserId();
        try {
            // Update metadata
    ...
    async addTodo(text, category, priority = 'medium', dueDate, userId) {
        ...
        return await this.withLock(effectiveUserId, async () => {
            ...
            const existingTodo = data.items.find(item => !item.completed && // Only check against incomplete todos
    ...
    async getIncompleteTodos(userId) {
    ...
    async getTodosByCategory(category, userId) {
        const todos = await this.getTodos(userId);
        return todos.filter(todo => todo.category === category);
    }
    async getTodosByPriority(priority, userId) {
        const todos = await this.getTodos(userId);
        return todos.filter(todo => todo.priority === priority);
    }
    async getDueTodos(userId) {
        const todos = await this.getTodos(userId);
    ...
    try {
        switch (request.params.name) {
            ...
            case "get_all": {
                ...
                const todoText = todoodles.map(t => `${t.id}. ${t.text} ${t.completed ? '✓' : '○'} [${t.priority}] ${t.category ? `(${t.category})` : ''} ${t.dueDate ? `Due: ${t.dueDate}` : ''}`).join('\n');
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "search": {
                ...
                const todoText = todoodles.map(t => `${t.id}. ${t.text} ${t.completed ? '✓' : '○'} [${t.priority}] ${t.category ? `(${t.category})` : ''}`).join('\n');
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "get_by_category": {
                ...
                const todoText = todoodles.map(t => `${t.id}. ${t.text} ${t.completed ? '✓' : '○'} [${t.priority}]`).join('\n');
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "get_by_priority": {
                ...
                const todoText = todoodles.map(t => `${t.id}. ${t.text} ${t.completed ? '✓' : '○'} ${t.category ? `(${t.category})` : ''}`).join('\n');
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "get_due": {
                const { overdue_only } = request.params.arguments;
                const todoodles = overdue_only
                    ? await todoodlesManager.getOverdueTodos(userId)
                    : await todoodlesManager.getDueTodos(userId);
                const todoText = todoodles.map(t => `${t.id}. ${t.text} [${t.priority}] Due: ${t.dueDate} ${t.category ? `(${t.category})` : ''}`).join('\n');
                return {
                    content: [
                        {
                            type: "text",
...
Last updated: ${stats.lastUpdated}`
            ...
            case "get_web_ui": {
                const { extend_minutes = 30 } = request.params.arguments;
